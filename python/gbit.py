...
# libgbit.so/.dll via Python's ctypes module
#
...
class Regs16(Structure):
    ...
    def __repr__(self):
        return (
            f"<Reg16 AF={self.AF:04x} BC={self.BC:04x} DE={self.DE:04x} "
            + f"HL={self.HL:04x}>"
        )
...
class Regs8(Structure):
    ...
    def __repr__(self):
        return (
            f"<Reg8 A={self.A:02x} F={self.F:02x} B={self.B:02x} "
...
class State(Structure):
    ...
    def __repr__(self):
        return (
            f"<CState {self.regs} SP={self.SP:04x} PC={self.PC:04x} "
...
class GBIT:
    def __init__(
        ...
        self.libname = libname or ("libgbit.so" if os.name != "nt" else "gbit.dll")
    ...
    def load_library(self):
        ...
        if not os.path.exists(self.libname):
            raise Exception(
                f"Could not find library {self.libname}. Make "
    ...
    def run(
        self,
        keep_going_on_mismatch=False,
        enable_cb_instructions=True,
        ...
        ops = Operations(
            self.c_init_cb, self.c_set_state_cb, self.c_get_state_cb, self.c_step_cb
        )
